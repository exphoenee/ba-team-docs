...
| `/add-answer "Q-XXX: ..."` | **Add answer** -- save stakeholder answers to `workflow/03_answers/`; smart backtick on missing Q-XXX ID, conflicts; no precondition checks |
| `/add-decision "text"` | **Add FORCED decision** -- create SDEC-XXX file in `workflow/04_decisions/` with YAML frontmatter; auto SDEC ID generation; smart backtick on missing targets, type |
