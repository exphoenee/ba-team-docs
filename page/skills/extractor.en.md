# `/extractor` – Specification Builder

[Magyar változat](README.md)

## What is it for?

The `/extractor` skill creates a **structured specification package suitable for decision-making** from unstructured, raw human communication — meeting notes, emails, Slack/Teams messages, and coordination records.
...
`/extractor` intelligently handles project changes:
...
- The skill now dispatches `extraction-agent` (formerly: `spec-builder-agent`). The `/extractor` command name has not changed — the change is internal.
